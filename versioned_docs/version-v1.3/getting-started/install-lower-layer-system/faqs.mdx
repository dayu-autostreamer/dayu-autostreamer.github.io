...
2. Check `clusterDNS` as required in [EdgeMesh Installation](/docs/v1.3/getting-started/install-lower-layer-system/install-edgemesh#configure-edge-network-edge).
...
**Solution:** Configure docker registry and restart docker and edgecore. Refer to [Docker Registry Configuration](/docs/v1.3/developer-guide/configure-docker-registry) for details.
...
Remember to modify `cloudcore.yaml` as [[Install KubeEdge]](/docs/v1.3/getting-started/install-lower-layer-system/install-kubeedge#configure-cloudcore) shows after `keadm init`.
...
Another temporary solution is to reinstall kubeedge on edge node as [instruction](/docs/v1.3/getting-started/install-lower-layer-system/install-kubeedge#join-kubeedge-cluster-edge) shows.
