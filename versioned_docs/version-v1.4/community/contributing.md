...
`docs/` and `i18n/zh/docusaurus-plugin-content-docs/current/` are the continuously updated current documentation.
Published
versions are frozen under `versioned_docs/version-<release>/` and the corresponding localized `version-<release>/`
directory. Apply later corrections to the current documentation; do not rewrite a published release snapshot. When a
Dayu release is cut, create and review matching English and Chinese snapshots and update the Docusaurus version
metadata together.
