...
- name: Frame Visualization  # Name of the visualization module
  type: image  # Supported types: image, curve, topology, cdf, gantt
  ...
  save_expense: true  # Whether to include hook execution expense, default is false
...
- name: Service Task Gantt
  type: gantt
  x_axis: Time
  y_axis: Service
  variables: [ "Service Task Timeline" ]
  hook_name: service_gantt
  size: 3

# ......
```

Gantt visualizers use the absolute execution timestamps stored for each task service. Use `service_gantt` to group by
service, or `service_device_gantt` with `hook_params` such as `'{"service": "traffic-detection"}'` to group one service
by execution device.
