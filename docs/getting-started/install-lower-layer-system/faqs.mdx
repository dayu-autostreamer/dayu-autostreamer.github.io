...
1. Verify `clusterDNS` as required in [EdgeMesh Installation](/docs/getting-started/install-lower-layer-system/install-edgemesh#configure-edge-network-edge).
...
**Solution:** Configure docker registry and restart docker and edgecore. Refer to [Docker Registry Configuration](/docs/developer-guide/configure-docker-registry) for details.
...
Another temporary solution is to reinstall kubeedge on edge node as [instruction](/docs/getting-started/install-lower-layer-system/install-kubeedge#join-kubeedge-cluster-edge) shows.
