...
2. 着重检查 [EdgeMesh安装](/docs/v1.3/getting-started/install-lower-layer-system/install-edgemesh#配置edge端网络边端)中的 `clusterDNS`
...
**解决：** 配置后重启docker和edgecore即可，具体配置详见[Docker镜像源配置](/docs/v1.3/developer-guide/configure-docker-registry)。
...
请注意, `keadm init`后需要按照[【安装 KubeEdge】](/docs/v1.3/getting-started/install-lower-layer-system/install-kubeedge#cloudcore-配置)章节的内容重新修改  `cloudcore.yaml` 文件。
...
另一种临时解决方案是参照[文档](/docs/v1.3/getting-started/install-lower-layer-system/install-kubeedge#加入kubeedge集群边端)在边缘节点上重新安装 KubeEdge 。
