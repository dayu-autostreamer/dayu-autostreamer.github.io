...
`position` 限制组件可渲染的位置。可选的 `cloud-pod-template` 与 `edge-pod-template` 会覆盖合并到
`pod-template`，用于设置节点类型相关的环境变量。Backend 会在创建 RuntimeService 前校验目标 slot、端口、容器引用、
host-path 类型与 target path。
