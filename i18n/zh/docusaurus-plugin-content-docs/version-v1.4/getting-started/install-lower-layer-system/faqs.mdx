...
✅ **检查大禹系统 Pod:** 使用 `kubectl get pods -n <your-dayu-namespace>` 检查大禹系统核心容器的状态；
并使用 `kubectl logs <pod-name> -n <your-dayu-namespace>` 和 `kubectl describes pod <pod-name> -n <your-dayu-namespace>` 排查异常状态容器的日志信息；

✅ **检查大禹系统核心资源:** 如果容器Pods在未知错误状态，可以使用 `kubectl get svc -n <your-dayu-namespace>` 和 `kubectl get deployment -n <your-dayu-namespace>` 检查 services 和 deployments 等核心资源状态；
...
## 问题十：Sedna LC 报错 `lookup gm.sedna on 127.0.0.53:53: no such host` {#question-10-sedna-lc-dns}

安装大禹系统配套的 Sedna 时，LC Pod 可能显示为 `Running`，但日志持续报错：
...
在当前配套的 Sedna 中，这还会阻止边缘 RuntimeService 完成激活。工作负载虽然已经运行，但由于 LC 无法从 GM 接收激活请求，资源会持续处于 `Activated=False`、`Ready=False` 和 `AwaitingNodeAck` 状态。

**原因：**

`dayu-sedna v1.1` 的 LC 有意使用 `hostNetwork: true`，以便通过 `127.0.0.1:10551` 核验本机 EdgeMesh 路由。如果 `edged.clusterDNS` 为空或配置错误，使用宿主机网络的 LC 会继承 `127.0.0.53` 等宿主机 DNS，而不是 `169.254.96.16` 上的 EdgeMesh DNS，因此无法解析 Kubernetes 服务名 `gm.sedna`。

> 保留当前 `dayu-sedna` LC 的 `hostNetwork`。移除该设置会使 LC 无法通过本机 EdgeMesh 路由完成 RuntimeService 路由确认。

**排查与解决：**

1. 检查哪些节点上的 LC 无法连接 GM：
...
2. 在每个异常边缘节点上检查 `/etc/kubeedge/config/edgecore.yaml`，生效配置必须包含：
...
3. 确认同一节点上的 EdgeMesh agent 已 Ready，且 EdgeDNS 已启动：
   ...
   健康的 agent 日志会显示 EdgeDNS 已运行并监听 `169.254.96.16:53`。

4. 修改 `edgecore.yaml` 后逐个节点重启 edgecore：
...
5. 重建对应节点上的 LC Pod，因为已有 Pod 会继续使用旧的 `/etc/resolv.conf`：

   ```bash
   kubectl delete pod -n sedna <lc-pod-name>
   ```

6. 验证最终状态，不能只检查 Pod 是否 Ready：
   ...
   LC 应使用 `169.254.96.16` 解析域名，日志应显示已成功建立 GM WebSocket 连接，带服务端点的 RuntimeService 应变为 `Activated=True` 和 `Ready=True`。

不要永久修改 `/etc/resolv.conf`；该文件由 Pod 运行环境生成，重启后修改可能丢失。应修复 `edgecore.yaml`。

## 问题十一：`169.254.96.16` 无法解析服务或请求超时 {#question-11-edgemesh-dns}

首先区分该故障与[问题十](#question-10-sedna-lc-dns)：

- 请求发往 `127.0.0.53` 表示 Pod 使用了错误的 DNS，应修复 `edged.clusterDNS`。
- 请求发往 `169.254.96.16` 但超时，表示该节点上的 EdgeDNS 不可用或不可达。
- `169.254.96.16` 返回 `NXDOMAIN`，通常表示 EdgeMesh 尚未同步对应的 Service 和 Endpoint 数据。

依次检查：

1. 按照 [EdgeMesh 安装](/docs/v1.4/getting-started/install-lower-layer-system/install-edgemesh#配置edge端网络边端)确认 `clusterDNS`。
2. 确认该节点的 `edgemesh-agent` 已 Ready，且日志显示 EdgeDNS 正常启动、没有持续报错。
3. 在异常边缘节点上检查 EdgeMesh 使用的边缘本地 API：

   ```bash
   curl -fsS --max-time 3 http://127.0.0.1:10550/api/v1/services
   ```

   如果端口 `10550` 拒绝连接，按照[问题三十二](#question-32-edgemesh-local-api)处理。

4. 按照[混合代理 | EdgeMesh](https://edgemesh.netlify.app/zh/advanced/hybird-proxy.html)检查 iptables 链。
5. 如果新加入节点无法获取 EdgeMesh 镜像，按照[问题二十四](#问题二十四16925496-1653-io-timeout)处理。
...
**解决：** 配置后重启docker和edgecore即可，具体配置详见[Docker镜像源配置](/docs/v1.4/developer-guide/configure-docker-registry)。
...
请注意, `keadm init`后需要按照[【安装 KubeEdge】](/docs/v1.4/getting-started/install-lower-layer-system/install-kubeedge#cloudcore-配置)章节的内容重新修改  `cloudcore.yaml` 文件。
...
对于大禹系统 RuntimeService，首先检查激活状态，不要直接判定为 iptables 残留：
...
如果工作负载已经运行，但带服务端点的 RuntimeService 仍为 `Activated=False`，原因为 `AwaitingNodeAck`，请按照[问题十](#question-10-sedna-lc-dns)处理。清空 edgecore 数据库无法修复已经断开连接的 Sedna LC，还可能造成不必要的数据丢失。
...
只有在 edgemesh-agent 日志明确显示 service port 冲突时，才检查边缘设备上的 iptables：
```bash
...
如果 RuntimeService 激活正常，并且确认存在上述残留规则，可以临时清空异常边缘节点上的 edgecore 数据库。该操作可能导致数据丢失，因此必须先备份：
```bash
# 停止 edgecore
...
另一种临时解决方案是参照[文档](/docs/v1.4/getting-started/install-lower-layer-system/install-kubeedge#加入kubeedge集群边端)在边缘节点上重新安装 KubeEdge 。

当前[大禹系统启动脚本](https://github.com/dayu-autostreamer/dayu/blob/main/dayu.sh)已经修复该问题。

## 问题二十九：JetPack 6 或 cgroup v2 上 edgecore 重启失败

在 JetPack 6 或其他 cgroup v2 系统上，`edgecore.service` 可能反复重启，并出现类似如下错误：
```bash
...
在 `/etc/kubeedge/config/edgecore.yaml` 中关闭按 QoS 创建 cgroup：
```bash
...
如果 edgecore 报错 `kubelet cgroup driver: "cgroupfs" is different from docker cgroup driver: "systemd"`，请在 `keadm join` 时加入 `--cgroupdriver=systemd`，或者在 `/etc/kubeedge/config/edgecore.yaml` 中设置 `cgroupDriver: systemd`。

然后重启 edgecore。如果旧的 `kubepods` slice 仍被 systemd 保留，可以先停止这些 slice，再启动 edgecore：
```bash
...
## 问题三十：NVIDIA device plugin 为 Running 但 GPU 资源未注册

`nvidia-device-plugin` pod 为 `Running` 只说明插件容器启动成功，并不代表 Kubernetes 已经为该边缘节点注册 GPU 资源。
...
预期输出应包含 `gpu=1 alloc=1` 或其他正数 GPU 数量。如果输出为空，请检查 NVIDIA device plugin 日志和 edgecore 的 GPU 开关：
```bash
...
然后重启 edgecore，并再次检查节点资源：
```bash
sudo systemctl restart edgecore.service
kubectl get node <edge-node-name> -o jsonpath='gpu={.status.capacity.nvidia\.com/gpu} alloc={.status.allocatable.nvidia\.com/gpu}{"\n"}'
```

## 问题三十一：边缘节点无法从 DockerHub 或 GitHub 下载内容

部分边缘节点无法直连 DockerHub 或 GitHub，或者在 `keadm join` 下载 KubeEdge release 包和 `kubeedge/pause:3.1` 镜像时持续超时。

**解决：**

在执行 `keadm join` 前，先预置 release 包和 pause 镜像。

1. 在可访问 GitHub 的机器上下载匹配架构的 KubeEdge release 包，然后拷贝到边缘节点：
```bash
scp keadm-v1.9.2-linux-arm64.tar.gz kubeedge-v1.9.2-linux-arm64.tar.gz <user>@<edge-node>:/home/<user>/
```

执行 join 时，`--tarballpath` 应填写包含 `kubeedge-v1.9.2-linux-arm64.tar.gz` 的目录，而不是 tarball 文件本身：
```bash
...
即使已经预置 release tarball，`keadm join` 仍可能访问 GitHub 下载 `checksum_kubeedge-v1.9.2-linux-arm64.tar.gz.txt` 和 `edgecore.service`。如果卡在 `checksum_kubeedge...` 或 `[Run as service] start to download service file for edgecore`，请确保 GitHub/raw.githubusercontent.com 可访问，或者通过临时本地镜像/wget wrapper 预置这两个小文件。如果使用 wrapper，join 完成后必须立刻恢复系统 `wget`。

2. 如果集群有内部镜像仓库，请在边缘节点上配置 Docker 使用该仓库。请将以下配置合并到 `/etc/docker/daemon.json` 中，不要覆盖已有 Docker 配置：
```json
{
...
最后确认本地已经存在 `kubeedge/pause:3.1`：
```bash
docker images | grep 'kubeedge/pause'
```

## 问题三十二：edgemesh-agent 为 Running 但日志中 127.0.0.1:10550 connection refused {#question-32-edgemesh-local-api}

`edgemesh-agent` pod 可能显示为 `Running`，但日志持续出现：
```bash
...
edgecore 没有开启边缘 Kube-API endpoint。此时 Edgemesh 可以启动，但无法从 `127.0.0.1:10550` watch Services 和 Endpoints。

**解决：**

在边缘节点的 `/etc/kubeedge/config/edgecore.yaml` 中开启 `edgeStream` 和 `metaServer`：
```yaml
edgeStream:
  enable: true
...
然后重启 edgecore 并检查本地 edge API：
```bash
sudo systemctl restart edgecore.service
curl -fsS --max-time 3 http://127.0.0.1:10550/api/v1/services
```

等待几秒后，`edgemesh-agent` 日志应出现 cache sync 或 iptables 更新信息，而不是持续刷 `10550 connection refused`。
