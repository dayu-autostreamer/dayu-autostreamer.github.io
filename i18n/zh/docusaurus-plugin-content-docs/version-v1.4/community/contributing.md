...
当大禹系统变更影响用户可见行为时，请在上游 issue、pull request 或 release 足够明确后同步更新文档。

### 版本生命周期

`docs/` 与 `i18n/zh/docusaurus-plugin-content-docs/current/` 是持续更新的当前文档。已发布版本冻结在
`versioned_docs/version-<release>/` 及对应的本地化 `version-<release>/` 目录中。后续修正应更新当前文档，
不要改写已发布的 release 快照。大禹系统发布新版本时，应同时创建并审阅中英文快照，并同步更新 Docusaurus 版本元数据。
...
- 本次变更解决了什么读者问题
- 内容适用于英文、中文还是两者都适用
- 对应哪个上游大禹系统 issue、pull request、release 或论文
- 本地是否已经运行 `npm run build`

文档类 pull request 通常需要文档站维护者 review。架构、调度器、运行时或部署教程相关内容，也可能需要对应的大禹系统负责人一起 review。
