...
✅ **检查 Dayu 核心资源:** 如果容器Pods在未知错误状态，可以使用 `kubectl get svc -n <your-dayu-namespace>` 和 `kubectl get svc -n <your-dayu-namespace>` 检查 services 和 deployments 等核心资源状态；
...
请注意, `keadm init`后需要按照[【安装 KubeEdge】]((/docs/getting-started/install-lower-layer-system/install-kubeedge#configure-cloudcore))章节的内容重新修改  `cloudcore.yaml` 文件。